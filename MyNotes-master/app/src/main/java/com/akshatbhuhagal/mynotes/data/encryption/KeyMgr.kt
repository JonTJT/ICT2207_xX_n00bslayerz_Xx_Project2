package com.akshatbhuhagal.mynotes.data.encryption
...
class KeyMgr {
    ...
    private fun initKey(passcode: CharArray, context: Context) {
        ...
        if (storable == null) {
            ...
            println("dbchar key: " + charArrayToHexString(dbCharKey!!))
        }
    }
    ...
    private fun getRawByteKey(passcode: CharArray, storable: Storable): ByteArray {
        ...
        println("Secret: " + secret.encoded.test())
        val cipher = Cipher.getInstance("AES/CTR/NoPadding")
        cipher.init(Cipher.DECRYPT_MODE, secret, IvParameterSpec(iv))
        println("Key: "+ cipher.doFinal(aesWrappedKey).test())
